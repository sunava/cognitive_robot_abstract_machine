...
A `set_of` is *searched* or *reported* by one plan-level shape, not a render-time `isinstance`: the {py:class}`~krrood.entity_query_language.verbalization.grammar.query.planner.QueryPlanner` sets a {py:class}`~krrood.entity_query_language.verbalization.grammar.query.planner.ReportPlan` when the selection computes an aggregate, splitting the GROUP BY keys out of the reported columns.  The assembler then dispatches on it: a search reads as natural prose *"Find a and b"* (Oxford-comma, no code-like parentheses); a report opens with *"Report …"*, and a grouped report fronts its grouping as *"For each <key>, report …"* — the grouping is the result's row dimension, so stating it first both reads as the GROUP BY idiom and (being fronted) drops the redundant key column and the trailing *"grouped by"* clause.  Group keys render as the bare group label (*"department"*, not *"the department of an Employee"* — there is no single employee).  A *ranked* set-of (`limit`) is excluded from the report shape and keeps the parenthesised *"Find the top three (a, b)"* form, whose pre-head needs the tuple as a unit.  *"report"* is held lowercase in the vocabulary and capitalised by the assembler only when it opens the sentence (there is no global sentence-case pass — leading capitals are positional in the keywords).
