Process Modules in Cora Plex
=========================

Process modules are Cora Plex’s bridge from symbolic intentions to concrete robot motions. They encapsulate the low‑level
...
The base class ``cora_plex.process_module.ProcessModule`` provides per‑module locking and an optional execution delay to make
...
of concrete modules is delegated to ``cora_plex.process_module.ProcessModuleManager`` and robot‑specific managers derived from
...
---------------------------------------------
Cora Plex switches execution context using the ``ExecutionType`` enum and light‑weight context helpers. Within a
``cora_plex.process_module.simulated_robot`` context (or when decorating a function with ``@with_simulated_robot``), the
manager returns simulated implementations. Within a ``cora_plex.process_module.real_robot`` context (or ``@with_real_robot``),
...
``cora_plex.process_modules.default_process_modules``) registers itself under the robot name ``"default"`` and implements a
...
--------------------------------
Beyond purely simulated and purely real contexts, Cora Plex provides a ``SemiRealRobot`` context for hybrid scenarios.
...
implement the relevant factory methods. Register the manager by importing it from ``cora_plex.process_modules`` so that
