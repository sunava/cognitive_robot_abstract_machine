...
from cora_plex.ros import create_publisher
...
class JointStatePublisher:
    ...
    def __init__(
        ...
        world: World,
        node,
        joint_state_topic="/cora_plex/joint_state",
        interval=0.1,
    ):
        """
        Robot object is from :py:attr:`~cora_plex.world.World.robot` and current joint states are published to
    ...
    def _publish(self) -> None:
        """
        Publishes the current joint states of the :py:attr:`~cora_plex.world.World.robot` in an infinite loop.
