...
(function () {
  ...
  function loadSlot(name, onReady) {
    ...
    fetch(base + 'scene.json').then(function (r) { return r.json(); }).then(function (sc) {
      ...
      // the two things that actually go wrong here are worth telling apart
      if (location.protocol === 'file:') {
        statusEl.innerHTML = 'Scenes cannot be read from a file:// page — the browser blocks it.' +
          '<br>Serve the folder instead: <code>python3 -m http.server</code> in defense/,' +
          '<br>then open <code>http://localhost:8000/index.html</code>';
      } else {
        sceneIndex.then(function (ix) {
          const listed = (ix && ix.scenes || []).map(function (s) { return s.name; });
          statusEl.innerHTML = 'Scene <b>' + name + '</b> is not in defense/scenes/.' +
            (listed.length ? '<br>scenes/index.json lists: ' + listed.join(', ') : '');
        });
      }
    });
  }
