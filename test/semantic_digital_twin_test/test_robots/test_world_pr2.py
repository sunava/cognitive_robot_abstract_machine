...
def test_from_world_reuses_existing_robot_in_copy(pr2_world_state_reset):
    copied_world = deepcopy(pr2_world_state_reset)

    existing_robot = copied_world.get_semantic_annotations_by_type(PR2)[0]
    recreated_robot = PR2.from_world(copied_world)

    assert recreated_robot is existing_robot
    assert copied_world.get_semantic_annotations_by_type(PR2).count(existing_robot) == 1
