...
A library that calls :func:`logging.basicConfig` at import time installs a root handler,
which turns every later ``basicConfig`` call into a silent no-op — so the console
scripts would print none of their startup output. ``cramera`` therefore leaves logging
to the entry points, and they configure it with ``force=True`` because other CRAM
packages do configure the root logger on import.
"""
...
IMPORT_AND_REPORT_ROOT_HANDLERS = (
    "import logging, cramera; print(logging.getLogger().handlers)"
)
...
class TestImportDoesNotConfigureLogging:
    ...
    def test_package_logger_propagates(self):
        ...
        """
        logger = logging.getLogger("cramera")
...
class TestEntryPointEmitsStartupOutput:
    ...
    def test_banner_survives_a_preconfigured_root_logger(
        ...
        server.main(["0"])
        assert "cramera running at" in capsys.readouterr().err

    def test_the_port_is_read_from_the_command_line_when_none_is_passed(
        self,
        isolated_logging,
        server_that_returns_immediately,
        capsys,
        fixture_scene,
        monkeypatch,
    ):
        """
        The console-script entry point takes its port from ``sys.argv``, since it is
        called with no arguments at all.
        """
        monkeypatch.setattr(sys, "argv", ["cramera", "0"])
        server.main()
        assert "cramera running at" in capsys.readouterr().err
