...
def _move_milk(milk: Body, *args: float, **kwargs: float) -> None:
    """
    Set the milk's parent-connection origin from ``from_xyz_rpy`` arguments, expressed
    in the parent's frame (the origin setter requires a reference frame).
    """
    milk.parent_connection.origin = HomogeneousTransformationMatrix.from_xyz_rpy(
        *args, reference_frame=milk.parent_connection.parent, **kwargs
    )
...
def test_contact_detector(_simple_apartment_setup):
    ...
    _move_milk(milk, box1.global_pose.x, box1.global_pose.y, box1.global_pose.z)
    segmind_executor.tick()
    assert len(events_of(segmind_context, ContactEvent)) == 1
    assert len(events_of(segmind_context, LossOfContactEvent)) == 0

    _move_milk(milk, box2.global_pose.x, box2.global_pose.y, box2.global_pose.z)
    segmind_executor.tick()
    ...
    _move_milk(milk, -1.7, 0, 1.07, yaw=np.pi)
...
def test_support_detector(_simple_apartment_setup):
    ...
    _move_milk(milk, box1.global_pose.x, box1.global_pose.y, box1.global_pose.z + 0.56)
    segmind_executor.tick()
    assert len(events_of(segmind_context, SupportEvent)) == 2
    assert len(events_of(segmind_context, LossOfSupportEvent)) == 1

    _move_milk(milk, box2.global_pose.x, box2.global_pose.y, box2.global_pose.z + 0.56)
    segmind_executor.tick()
...
def test_translation(_simple_apartment_setup):
    ...
    for i in range(5):
        _move_milk(milk, x=1 + i * 0.1, y=-3, z=0.25)
        segmind_executor.tick()
...
def test_slow_motion_with_all_motion_detectors(_simple_apartment_setup):
    ...
    _move_milk(milk, x=1, y=-3, z=0.25)
    for _ in range(8):
        segmind_executor.tick()
    ...
    for i in range(1, 9):
        _move_milk(milk, x=1 + i * 0.002, y=-3, z=0.25, roll=i * 0.04)
        segmind_executor.tick()
